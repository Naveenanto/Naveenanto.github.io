...
;( function( window ) {
		...
		this.navDots.forEach( function( dot, idx ) {
			dot.addEventListener( 'click', function() {
				//if( idx !== self.current ) {
					//self._showItem( idx );
				//}
                console.log(self._showItem(idx));
                console.log(idx);
			} );
		} );
	}

    TiltSlider.prototype._startInteraction = function() {
        var self = this;
       var items =  [].slice.call(document.getEkegetElementById("subMenuRule").children);
        console.log(items);
        items.forEach(function(dot,idx){
            dot.addEventListener( 'click', function() {
                if( idx !== self.current ) {
                    console.log(self._showItem( idx ));
                }
                //console.log(self._showItem(idx+1));
            } );
        })
    }
	...
	//	classie.removeClass( this.navDots[ this.current ], 'current' );
        ...
        console.log("current = "+ this.current+ " next = "+pos);
